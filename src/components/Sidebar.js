// Operating System Sidebar Component
...
export function renderSidebar(container, { currentView, onNavigate, isCollapsed, onToggleCollapse }) {
  ...
  navSections.forEach(sec => {
    navHtml += `
      <div class="nav-section">
        ...
        ${sec.items.map(item => `
          <div class="nav-item ${currentView === item.id ? 'active' : ''}" data-view="${item.id}" title="${item.label}">
  ...
  document.getElementById('sidebar-toggle-btn').onclick = onToggleCollapse;
}
